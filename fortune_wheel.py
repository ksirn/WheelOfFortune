...
class MediaGalleryDialog(QDialog):
    """Мини-галерея медиафайлов для центра колеса."""

    selected = pyqtSignal(str)   # path to selected file

    def __init__(self, current: Optional[str], dark: bool, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Галерея медиа")
        self.setMinimumSize(580, 420)
        self.dark_mode = dark
        self.current_path = current
        self._movies: List[QMovie] = []   # keep alive

        p = PALETTE["dark" if dark else "light"]
        self.setStyleSheet(f"""
            QDialog {{ background: {p['bg']}; color: {p['text']}; font-family: 'Segoe UI'; }}
            ...
            QPushButton#primary {{
                background: {p['accent']}; color: #fff; border: none; font-weight: 700;
            }}
            QPushButton#primary:hover {{ background: {p['spin_end']}; }}
            QPushButton#danger {{ background: {p['surface2']}; color: {p['accent2']}; border-color: {p['accent2']}; }}
            QLineEdit {{
                background: {p['surface2']}; color: {p['text']};
                border: 1px solid {p['border']}; border-radius: 6px;
        ...
        top = QHBoxLayout()
        top.setSpacing(8)
        lbl = QLabel("Выберите медиафайл для центра колеса")
        lbl.setStyleSheet(f"font-size: 13px; font-weight: 700; color: {p['text']};")
        top.addWidget(lbl)
        top.addStretch()

        add_file_btn = QPushButton("+ Файл с диска")
        add_file_btn.clicked.connect(self._add_from_disk)
        top.addWidget(add_file_btn)

        add_url_btn = QPushButton("+ По ссылке")
        add_url_btn.clicked.connect(self._add_from_url)
        top.addWidget(add_url_btn)

        none_btn = QPushButton("Без медиа")
        ...
        self._grid_w.setStyleSheet(f"background: {p['surface']};")
    ...
    def _build_grid(self):
        ...
        files = [f for f in files if f.suffix.lower() in {".gif", ".png", ".jpg", ".jpeg", ".webp"}]

        COLS = 4
        for i, fpath in enumerate(files):
            card = QWidget()
            is_selected = self.current_path and Path(self.current_path) == fpath
            card.setStyleSheet(f"""
                QWidget {{
                    background: {p['surface2']};
                    border: 2px solid {p['accent'] if is_selected else p['border']};
                    border-radius: 8px;
                }}
            """)
            ...
            # Thumbnail
            thumb = QLabel()
            thumb.setFixedSize(110, 90)
            thumb.setAlignment(Qt.AlignmentFlag.AlignCenter)
            thumb.setStyleSheet("border: none; background: transparent;")

            if fpath.suffix.lower() == ".gif":
                movie = QMovie(str(fpath))
                movie.setScaledSize(QSize(110, 90))
                thumb.setMovie(movie)
                movie.start()
                self._movies.append(movie)
            else:
                px = QPixmap(str(fpath)).scaled(110, 90, Qt.AspectRatioMode.KeepAspectRatio,
                                                 Qt.TransformationMode.SmoothTransformation)
                thumb.setPixmap(px)

            cl.addWidget(thumb)

            name_lbl = QLabel(fpath.name[:16] + ("…" if len(fpath.name) > 16 else ""))
            name_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            name_lbl.setStyleSheet(f"font-size: 10px; color: {p['muted']}; border: none; background: transparent;")
            cl.addWidget(name_lbl)

            btn_row = QHBoxLayout()
            btn_row.setSpacing(3)
            sel_btn = QPushButton("✓" if is_selected else "Выбрать")
            sel_btn.setObjectName("primary" if is_selected else "")
            sel_btn.setFixedHeight(22)
            sel_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {p['accent'] if is_selected else p['surface2']};
                    color: {'#fff' if is_selected else p['text']};
                    border: 1px solid {p['accent'] if is_selected else p['border']};
                    border-radius: 4px; font-size: 10px; font-weight: 700;
                }}
                QPushButton:hover {{ background: {p['spin_end']}; color: #fff; border-color: {p['spin_end']}; }}
            """)
            path_str = str(fpath)
            sel_btn.clicked.connect(lambda _, pp=path_str: self._select(pp))
            ...
            del_btn.setStyleSheet(f"""
                QPushButton {{
                    ...
                    color: {p['muted']}; font-size: 11px; border-radius: 4px;
                }}
                QPushButton:hover {{ color: {p['accent2']}; background: {p['surface']}; }}
            """)
            del_btn.clicked.connect(lambda _, pp=path_str: self._delete(pp))
        ...
        if not files:
            empty = QLabel("Нет медиафайлов.\nДобавьте GIF, PNG или JPG.")
    ...
    def _add_from_disk(self):
        paths, _ = QFileDialog.getOpenFileNames(
            ...
            "Медиафайлы (*.gif *.png *.jpg *.jpeg *.webp)"
        )
        for path in paths:
            dst = MEDIA_DIR / Path(path).name
            if not dst.exists():
                shutil.copy2(path, dst)
        self._build_grid()

    def _add_from_url(self):
        url, ok = QInputDialog.getText(self, "Добавить по ссылке", "Вставьте URL (GIF/PNG/JPG):")
        ...
        try:
            ...
            fname = f"media_{int(time.time())}{ext}"
            dst = MEDIA_DIR / fname
...
class WheelWidget(QWidget):
    ...
    def set_media(self, path: Optional[str]):
        if self._movie:
            self._movie.stop()
            self._movie = None
        self._pixmap = None
        self._media_path = path or None

        if not self._media_path or not Path(self._media_path).exists():
            self.update()
            return

        if Path(self._media_path).suffix.lower() == ".gif":
            self._movie = QMovie(self._media_path)
            self._movie.frameChanged.connect(lambda _: self.update())
            self._movie.start()
        else:
            self._pixmap = QPixmap(self._media_path)
        self.update()
    ...
    def paintEvent(self, _):
        ...
        for lot, prob in zip(self.lots, self.probs):
            ...
            # Available pixel width along the arc at 95% radius
            arc_len_px = int(radius * 1.5 * math.radians(span) * 1.5)

            # Measure actual text width using font metrics — no guessing
            full_w = fm.horizontalAdvance(lot.name)
            if full_w <= arc_len_px:
                label = lot.name
            else:
                # Binary-search the longest prefix that fits with ellipsis
                lo, hi = 0, len(lot.name)
                label = ""
                while lo <= hi:
                    mid = (lo + hi) // 2
                    candidate = lot.name[:mid] + "…"
                    if fm.horizontalAdvance(candidate) <= arc_len_px:
                        label = candidate
                        lo = mid + 1
                    else:
                        hi = mid - 1
                if fm.horizontalAdvance(label) > arc_len_px or len(label) <= 2:
                    continue
            ...
            text_rect = QRectF(-160, -13, 320, 26)
        ...
        # ── Center circle ──
        cr = 136   # larger circle for media
        is_hover = self.hover_center and not self.spinning

        # Glow
        glow = QColor(p["accent"])
        glow.setAlpha(70 if is_hover else 20)
        grad = QRadialGradient(cx, cy, cr + 24)
        grad.setColorAt(0, glow)
        grad.setColorAt(1, QColor(0, 0, 0, 0))
        painter.setBrush(QBrush(grad))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawEllipse(QPointF(cx, cy), cr + 24, cr + 24)

        # Circle background
        fill = QColor(p["center_bg"])
        painter.setBrush(QBrush(fill))
        bc = QColor(p["accent"])
        bc.setAlpha(180 if is_hover else 120)
        painter.setPen(QPen(bc, 2.5))
        painter.drawEllipse(QPointF(cx, cy), cr, cr)

        # Clip to circle for media
        clip_path = QPainterPath()
        clip_path.addEllipse(QPointF(cx, cy), cr - 2, cr - 2)
        painter.setClipPath(clip_path)

        has_media = (self._movie or self._pixmap) and not self.spinning

        if has_media:
            # Draw current frame of GIF or static image
            if self._movie:
            ...
            if frame and not frame.isNull():
                ...
                painter.drawPixmap(int(cx - cr + 2), int(cy - cr + 2), cropped)
            ...
            if is_hover:
                ...
                painter.setPen(Qt.PenStyle.NoPen)
                painter.drawEllipse(QPointF(cx, cy), cr - 2, cr - 2)
                painter.setClipping(False)
                painter.setPen(QColor("#ffffff"))
                painter.setFont(QFont("Segoe UI", 26))
                painter.drawText(QRectF(cx - cr, cy - cr, cr * 2, cr * 2),
        ...
        # Border on top of media
        painter.setBrush(Qt.BrushStyle.NoBrush)
        bc2 = QColor(p["accent"])
        bc2.setAlpha(200 if is_hover else 140)
        painter.setPen(QPen(bc2, 2.5))
        painter.drawEllipse(QPointF(cx, cy), cr, cr)
    ...
    def mousePressEvent(self, e):
        cx, cy = self.width() / 2, self.height() / 2
        if math.hypot(e.position().x() - cx, e.position().y() - cy) <= 136 and not self.spinning:
            self.clicked.emit()
...
class FortuneWheelApp(QMainWindow):
    ...
    def _load_data(self):
        ...
        self.media_path = data.get("media_path") or None
    ...
    def _build_ui(self):
        ...
        self.gallery_btn = QPushButton("🖼 Медиа")
    ...
    def _open_gallery(self):
        dlg = MediaGalleryDialog(self.media_path, self.dark_mode, self)
        dlg.selected.connect(self._on_media_selected)
        dlg.exec()

    def _on_media_selected(self, path: str):
        self.media_path = path if path else None
        self.wheel.set_media(self.media_path)
        self._save_data()
        self._apply_styles()   # update gallery button color
    ...
    def _apply_styles(self):
        ...
        has_media = bool(self.media_path and Path(self.media_path).exists())
        self.gallery_btn.setStyleSheet(f"""
            QPushButton {{
                background: {p['accent'] if has_media else p['surface2']};
                color: {'#fff' if has_media else p['text']};
                border: 1px solid {p['accent'] if has_media else p['border']};
                border-radius: 6px; padding: 0 11px; font-size: 12px;
            }}
            QPushButton:hover {{ border-color: {p['accent']}; color: {'#fff' if has_media else p['accent']}; }}
        """)
